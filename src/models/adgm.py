...
class adgm(model):
    ...
    def __init__(self,n_x,n_y,n_h,n_z=2,n_a=2,x_dist='Gaussian',mc_samples=1,alpha=0.1,l2_reg=0.3,learning_paradigm='supervised',name = 'adgm',ckpt=None,prior=None,output_dir=None,analytic_kl=False, loss_balance='average', model_name='adgm'):
        ...
        super(adgm, self).__init__(n_x, n_y, n_h, n_z, x_dist, mc_samples, l2_reg,
    ...
    def build_model(self):
        ...
        # self.p_z_y_mean.add(Dense(self.n_z,input_dim=self.n_y,kernel_initializer='VarianceScaling', bias_initializer='zeros'))
        # self.p_z_y_log_var=Sequential()
        # self.p_z_y_log_var.add(Dense(self.n_z, activation='softplus',input_dim=self.n_y,kernel_initializer='VarianceScaling', bias_initializer='zeros'))
        ...
        self.p_a_xyz_model.add(Dense(self.intermediate_dim, kernel_initializer=glorot_initializer, bias_initializer=normal_initializer, input_dim=self.n_x+self.n_z+self.n_y))
        ...
        if self.x_dist == 'Gaussian':
            ...
            self.p_x_yz_model.add(Dense(self.intermediate_dim, kernel_initializer=glorot_initializer, bias_initializer=normal_initializer, input_dim=self.n_z+self.n_y))
            ...
            self.p_x_yz_mean_model.add(Dense(self.intermediate_dim, kernel_initializer=glorot_initializer, bias_initializer=normal_initializer, input_dim=self.n_z+self.n_y))
    ...
    def labelled_loss(self, x, y):
        ...
        l_loss = self.lowerBound(x_, y_, z, z_m, z_lv, a, a_m, a_lv)
    ...
    def unlabelled_loss(self, x):
        ...
        n_u = tf.shape(x)[0] 
        lb_u = tf.transpose(tf.reshape(self.lowerBound(x_, y_, z, z_m, z_lv, a_r, a_mr, a_lvr), [self.n_y, n_u]))
        lb_u = tf.reduce_sum(qy_l * lb_u, axis=-1)
        qy_entropy = -tf.reduce_sum(qy_l * tf.log(qy_l + 1e-10), axis=-1)
        return lb_u + qy_entropy

    def lowerBound(self, x, y, z, z_m, z_lv, a, a_m, a_lv):
    ...
    def _printing_feed_dict(self, Data, x_l, x_u, y, eval_samps, binarize):
        fd = super(adgm_k,self)._printing_feed_dict(Data, x_l, x_u, y, eval_samps, binarize)
