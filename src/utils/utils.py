...
def h_opt_args_shuffle(args):
    args.number_of_dims_z = np.random.randint(50,600)
    args.number_of_dims_a = np.random.randint(50,600)
    args.number_of_units_in_hidden_layers = np.random.randint(100,700)
    args.batch_size = np.random.randint(50,3000)
    args.number_of_mc_samples = np.random.choice([1,2,5])
    args.greyscale = np.random.choice([True, False])
    args.PCA = np.random.choice([True, False])
    args.normalize = np.random.choice([True, False])
    if args.greyscale == True:
            args.number_of_units_in_hidden_layers = np.random.randint(100,700)
    elif args.greyscale == False:
        if args.PCA == True:
            args.number_of_units_in_hidden_layers = np.random.randint(100,700)
        elif args.PCA == False:
            args.number_of_units_in_hidden_layers = np.random.randint(100,1500)
    return args

def h_opt_args_shuffle_reduced(args):
    args.number_of_dims_z = np.random.randint(50,600)
    args.number_of_dims_a = np.random.randint(50,600)
    args.number_of_units_in_hidden_layers = np.random.randint(100,1200)
    args.batch_size = np.random.randint(50,3000)
    return args
...
def make_token(args):
    ...
    #token_list_keys = args.__dict__.keys()
    token_list.reverse()
    #token_list_keys.reverse()
    #token_list_joined = [a+'_'+str(b) for a, b in zip(token_list_keys, token_list)]
